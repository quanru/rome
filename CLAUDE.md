...
- After UI changes in the [shadcn lint scan scope](DEVELOPMENT.md#shadcn-lint), run `pnpm lint:shadcn`. Review findings in changed files and report actionable findings, known false positives, and coverage limits in the handoff. Do not treat exit code 0 as zero findings.
- When reviewing CI, inspect the `shadcn lint` check and its summary for counts, locations, and repair suggestions. Findings fail the check without blocking merging. Treat an incomplete scan as unavailable, never clean. Do not rewrite valid tokens or unrelated UI to silence this check.
