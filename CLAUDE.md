...
- Before opening or updating a PR with UI changes, follow the [design-system self-check](docs/authoring/prs.md#design-system-self-check). The shadcn CI job is disabled, so agents must review lint findings locally and record the result in the PR test plan.
