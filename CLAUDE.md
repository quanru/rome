...
- When reviewing CI, read the finding count in the `shadcn lint` result name even if CI is green. Inspect source-line warnings and the `Scan shadcn usage` summary for locations and repair suggestions. Treat an incomplete scan as unavailable, never clean. Do not rewrite valid tokens or unrelated UI to silence this check.
