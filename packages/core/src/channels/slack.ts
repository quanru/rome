...
export type SlackIngressHandler = (envelope: SlackEventEnvelope) => void | Promise<void>;
...
export class SlackIngress {
  private readonly handlers = new Map<string, Set<SlackIngressHandler>>();
  ...
  subscribe(teamId: string, handler: SlackIngressHandler): () => void {
    let teamHandlers = this.handlers.get(teamId);
    if (!teamHandlers) {
      teamHandlers = new Set();
      this.handlers.set(teamId, teamHandlers);
    }
    teamHandlers.add(handler);
    return () => {
      teamHandlers?.delete(handler);
      if (teamHandlers?.size === 0) this.handlers.delete(teamId);
    };
  }
  ...
  async dispatch(envelope: SlackEventEnvelope): Promise<"delivered" | "duplicate" | "unhandled"> {
    ...
    if (!handlers || handlers.size === 0) return "unhandled";
...
export class SlackAdapter {
  ...
  async start(): Promise<void> {
    ...
    this.identity = await api.authTest(this.config.botToken);
    this.unsubscribe = this.config.ingress.subscribe(this.identity.teamId, (envelope) => {
      this.handleEnvelope(envelope);
    });
    log.info("slack adapter started", {
      teamId: this.identity.teamId,
      botUserId: this.identity.botUserId,
    });
  }
...
export function waitForSlackGuardianLink(
  ...
  return new Promise((resolve, reject) => {
    ...
    unregister = ingress.subscribe(identity.teamId, (envelope) => {
      if (!isSlackGuardianLinkEvent(envelope, code, identity.botUserId)) return;
