 ...
 * - `targets` are the picker's own eligible people, the guardian excluded and
 *   linked accounts folded into the person that holds them — so a recommended
 *   target is always one the manual Link flow could already reach, never a new
 *   one this invents.
 ...
 */
export function recommendedLinkTargets(
  row: PeopleRow,
  ...
  if (key === "") return [];
  return targets.filter((target) => normalizeDisplayName(target.displayName) === key);
}
