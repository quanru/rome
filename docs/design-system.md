...
Use semantic tokens in components: `bg-background`, `text-foreground`, `bg-surface`, `text-muted-foreground`, `border-border`, `bg-primary`, `text-destructive`, and `bg-success`/`bg-warning`/`bg-info`. A raised container — a card, a panel, a dialog — outlines itself with `border-edge` and `shadow-surface` together, not with `border-border`; the divider between rows stays `border-border`. Ash resolves the pair to nearly no edge and a soft cast, Ember and Slate to a hairline and no cast, so a container that writes `border-border` keeps its line in the one theme that removed lines. [`ui/semantic-token/edges.md`](ui/semantic-token/edges.md) is the contract. The dialog and sheet scrim is `bg-overlay` (token `--overlay`), which bakes the alpha into the token. Use `bg-overlay` at full opacity rather than `bg-black/40`. The destructive and error token is `destructive`, not `danger`.
