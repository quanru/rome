...
The bot scopes are deliberately limited to `chat:write`, `im:history`, and `app_mentions:read`. Subscribe only to `message.im` and `app_mention`. Do not add channel-message subscriptions: Rome must not receive unmentioned channel traffic.
The manifest also enables a writable App Home Messages tab so workspace members can find and direct-message the bot.

Slack signs every Events API request. Rome verifies the signature against the untouched request body, rejects timestamps older than five minutes, and checks the event's workspace against the workspace reported by the installed bot token.
