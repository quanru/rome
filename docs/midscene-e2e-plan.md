...
- **密钥与信任边界**：workflow **不配置任何 environment、不监听 pull_request**（PR 检出的是贡献者可控代码，而声明一个并不存在的受保护 environment 不会自动产生审批门——缺省会被无门创建）。四个 `MIDSCENE_MODEL_*` 经 repository secrets 注入，且只挂在「类型检查后的配置校验 / 连通性预检 / 跑用例」相关步骤上（安装与起服务不接触密钥）；作业以 `if: github.ref == 'refs/heads/main'` fail-closed，密钥只可能在受保护默认分支的代码上运行。将来要让 PR 跑，必须先由维护者在 Settings 真正创建带 required reviewers 的受保护 environment（建议同时改用 environment 级密钥），再在 workflow 加回经该 environment 审批的 `pull_request`。作业开头做非空校验与一次 `/chat/completions` 连通性预检（90s 超时），密钥/端点问题在 30 秒内失败而不是跑满 45 分钟。
- **网络可达性（重要）**：预检会从 runner 直连 `MIDSCENE_MODEL_BASE_URL`。托管的 `ubuntu-24.04` 公共 runner 只能访问公网端点；若模型网关位于内网（如公司内网域名），公共 runner 会连接超时，此时必须改用能访问该内网的 **self-hosted runner**（改 `runs-on` 标签）或换一个公网可达的模型端点——密钥正确但网络不通时预检同样快速失败。
